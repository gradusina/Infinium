...
namespace Infinium
{
    public partial class MuttlementDownloadForm : Form
    {
        ...
        public MuttlementDownloadForm(int iMutualSettlementDocumentID, string sFileName, ref MutualSettlements tMutualSettlementsManager)
        {
            InitializeComponent();

            MutualSettlementDocumentID = iMutualSettlementDocumentID;
            FileName = sFileName;
        ...
        private void OpenButton_Click(object sender, EventArgs e)
        {
            ...
            { temppath = MutualSettlementsManager.OpenMutualSettlementDocument(MutualSettlementDocumentID); });
            T.Start();
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            if (FileName.Length > 0)
            {
                ...
                if (saveFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    T = new System.Threading.Thread(delegate()
                    { MutualSettlementsManager.SaveMutualSettlementDocument(MutualSettlementDocumentID, saveFileDialog1.FileName); });
                    T.Start();
                    ...
                    Security.SetMuttlementSaveFilePath(Path.GetDirectoryName(saveFileDialog1.FileName));
                    Security.MuttlementSaveFilePath = Path.GetDirectoryName(saveFileDialog1.FileName);
